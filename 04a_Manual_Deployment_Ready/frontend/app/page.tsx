...
export default function Home() {
  ...
  const checkBackendHealth = useCallback(async () => {
    try {
      ...
      setConnectionError(`Backend health check failed: ${error}`);
      return false;
    }
  }, []);
    ...
    try {
      ...
      ws.onerror = (error) => {
        ...
        const errorMessage = isSecure 
          ? 'Failed to connect to story service (HTTPS/WSS connection)'
          : 'Failed to connect to story service (HTTP/WS connection)';
        setConnectionError(`${errorMessage}. Check console for details.`);
        setIsConnecting(false);
        setIsGenerating(false);
        setIsGeneratingImages(false);
        setImageGenerationStatus('');
  ...
  }, [connectWebSocket, checkBackendHealth]);
