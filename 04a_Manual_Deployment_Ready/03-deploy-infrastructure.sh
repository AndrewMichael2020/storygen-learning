...
# ---- Normalize env names (accept either old or new) ----
: "${BUCKET_NAME:=${GENMEDIA_BUCKET:-}}"
: "${SECRET_NAME:=${SECRET_MANAGER:-}}"
: "${ARTIFACT_REPO:=${GAR_REPOSITORY:-storygen-repo}}"
...
: "${BUCKET_NAME:?BUCKET_NAME or GENMEDIA_BUCKET is required}"
: "${SECRET_NAME:?SECRET_NAME or SECRET_MANAGER is required}"
...
# (Removed) Dead import of a deleted module.
# Previously tried to import: module.generated-images-bucket.google_storage_bucket.bucket

# Check if Artifact Registry repo exists (info only)
...
# Check if Secret Manager secret exists (info only)
...
gcloud run services update "$BACKEND_SERVICE_NAME" \
    --region="$REGION" \
    --project="$PROJECT_ID" \
    --update-labels="dev-tutorial=codelab-annie-devfest" || true
...
echo "   Actual backend URL: ${ACTUAL_BACKEND_URL:-<none>}"
...
if [ -n "$ACTUAL_BACKEND_URL" ] && [ "$CURRENT_FRONTEND_BACKEND" != "$ACTUAL_BACKEND_URL" ]; then
...
# Save outputs for easy access (keys match main.tf outputs)
